...
- **modal/task 依存と O\* 異常**: 正常反応の MAE_normal では SevenNet の
  mpa / omat24 / matpes_pbe / oc20 が 0.19–0.20 eV で拮抗する。一方 MAE_total では、
  **学習データ・task・head に Hubbard +U が含まれる modal/task ほど金属表面 O\* の anomaly が
  増え**、総合が悪化する傾向がある。実際、Materials Project 系（PBE+U 混在）データ由来の
  `mpa`(MPtrj) や `omat24`(OMat24) は O\* で崩れやすく MAE_total が 0.67–0.70 eV に悪化する
  のに対し、+U を含まない `matpes_pbe`(MatPES) や `oc20`(OC20／金属表面) は O\* が安定で
  MAE_total も最小（matpes_pbe 0.249 eV）。UMA も oc20/oc25/omat は中上位だが `oc22` は
  この合金×小分子系に不適合で最下位。この「+U 由来の PBE/PBE+U PES 混在による金属 O\* 異常」の
  詳細と論文引用（arXiv:2510.11241 / arXiv:2601.21056）は
  [総合まとめ](../README_jp.md) を参照。
